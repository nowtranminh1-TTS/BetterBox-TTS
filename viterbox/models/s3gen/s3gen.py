...
class S3Token2Mel(torch.nn.Module):
    ...
    def forward(
        ...
        if ref_dict is None:
            ...
            # Tạo bản sao cục bộ để không mutate `ref_dict` gốc giữa nhiều lần infer.
            normalized_ref_dict = {}
            for rk, rv in ref_dict.items():
                ...
                normalized_ref_dict[rk] = rv
            ref_dict = normalized_ref_dict
...
class S3Token2Wav(S3Token2Mel):
    ...
    def inference(
        ...
        finalize: bool = True,
        # Mel interpolation speed control (tính năng gốc từ Chatterbox, trước đây bị bỏ qua)
        # speed > 1.0 → nén mel → nói nhanh hơn | speed < 1.0 → giãn mel → nói chậm hơn
        # Khác với librosa.time_stretch: thao tác ở mel-level giữ nguyên pitch, chất lượng tốt hơn
        speed: float = 1.0,
    ):
        output_mels = self.flow_inference(speech_tokens, ref_wav=ref_wav, ref_sr=ref_sr, ref_dict=ref_dict, finalize=finalize)

        # ── Mel interpolation speed control ────────────────────────────────
        # Implement tính năng speed control mà tác giả Chatterbox đã TODO:
        #   "ignoring the speed control (mel interpolation) ... for now"
        # Scale chiều thời gian của mel-spectrogram trước khi vào HiFiGAN vocoder
        if speed != 1.0:
            speed = max(0.5, min(2.0, speed))  # clamp an toàn
            # output_mels shape: (batch, n_mels, time)
            new_time = int(output_mels.shape[2] / speed)
            if new_time > 0:
                output_mels = torch.nn.functional.interpolate(
                    output_mels,
                    size=new_time,
                    mode='linear',
                    align_corners=False,
        ...
        # Tách graph rõ ràng ở biên inference để giảm giữ tham chiếu tensor không cần thiết.
        return output_wavs.detach(), output_sources.detach()
