...
class T3(nn.Module):
    ...
    def inference(
        ...
        *,
        ...
        #self.compiled = False # dòng này khiến hàm if phía dưới luôn chạy -> luôn tạo patched_model mới
        ...
        # Trường hợp EOS đến ngay bước đầu: tránh torch.cat([]) gây crash.
        if len(predicted) == 0:
            predicted_tokens = torch.empty((1, 0), dtype=torch.long, device=device)
        else:
            predicted_tokens = torch.cat(predicted, dim=1)  # shape: (B, num_tokens)

        # Dọn thêm các tensor trung gian
        del output, inputs_embeds, embeds, generated_ids, predicted
        del bos_token, bos_embed

        # Dọn KV cache sau khi xong - tránh lỗi tích lũy khi chữ dài
        del past
        if torch.cuda.is_available():
            torch.cuda.synchronize()  # đợi CUDA ops xong hẳn
