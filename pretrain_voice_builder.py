...
Kết quả sau khi build là file 'conds.pt' trong folder 'output-profile/'.
Nhấn 'Copy → modelTTSLocal' để app dùng ngay (cần restart app).
...
CÁCH SỬ DỤNG:
    1. Đặt audio (.wav/.mp3/.flac/...) cùng 1 giọng vào folder 'pretrained/'
    ...
    4. Nhấn "📋 Copy → modelTTSLocal" để dùng ngay làm default
──────────────────────────────────────────────────────────────────────────────
"""
...
MODEL_DIR       = _ROOT / "modelTTSLocal"    # Folder model gốc
...
def build_voice_profile(
    ...
    need_unload = False
    if model is None:
        log_fn("\n🤖 Đang load model từ modelTTSLocal/...")
        try:
    ...
    summary = (
        ...
        f"   💾 Đã lưu: {out_path}\n"
        f"\n"
        f"📌 Nhấn 'Copy → modelTTSLocal' để dùng ngay làm default.\n"
...
def copy_profile_to_model(
    ...
    """
    Copy conds.pt từ output-profile/ sang modelTTSLocal/ để dùng ngay làm default.
    ...
    if not src.exists():
        return "❌ Chưa có conds.pt trong output-profile/. Hãy Build Voice Profile trước."
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Build voice conditioning profile v3 từ folder pretrained/"
    )
    ...
    parser.add_argument("--copy_to_model",  action="store_true",
                        help="Tự động copy conds.pt sang modelTTSLocal/ sau khi build")
